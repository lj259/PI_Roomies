...
<div class="d-flex">
    ...
    <!--Campos de Editar Aviso-->
    <div class="container-fluid p-4 justify-content-around border">
        <h3 class="text-center mb-4">Editar un aviso</h3>
        <div class="col-4 mx-auto">
            <div class=""></div>
            <form method="POST" action="/Admin/Avisos/update/{{$aviso->id}}">
                @csrf
                @method('PUT')
                <div class="form-group mt-3">
                    <label for="titulo">Título</label>
                    <input type="text" class="form-control" value="{{$aviso->titulo}}" name="titulo"
                        placeholder="Titulo del aviso">
                    <small class="text-danger fst-italic">{{$errors->first('titulo')}}</small>
                </div>
                <div class="form-group mt-3">
                    <label for="descripcion">Descripción</label>
                    <input type="text" class="form-control" value="{{$aviso->descripcion}}" name="descripcion"
                        placeholder="Descripción del aviso">
                    <small class="text-danger fst-italic">{{$errors->first('descripcion')}}</small>
                </div>
                <div class="form-group mt-3">
                    <label for="estado">Estado</label>
                    <input type="text" class="form-control" value="{{$aviso->activo}}" name="estado"
                        placeholder="Activo o inactivo">
                    <small class="text-danger fst-italic">{{$errors->first('estado')}}</small>
                </div>
                <div class="form-group mt-3">
                    <label for="banos">Categoría</label>
                    <input type="text" class="form-control" value="{{$aviso->id_categoria}}" name="categoria"
                        placeholder="xd">
